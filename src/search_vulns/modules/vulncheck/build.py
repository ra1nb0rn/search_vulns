...
async def vulncheck_worker(cveids, headers, rate_limit):
...
async def full_update_async(productdb_config, vulndb_config, module_config, stop_update):
    ...
    if vulncheck_api_key:
        ...
        # update: vulncheck API now has a rate limit of 1000 requests per minute: https://docs.vulncheck.com/api#rate-limiting
        ...
        while offset < len(vulncheck_cves):
            ...
            task = asyncio.create_task(vulncheck_worker(cveids=cveids, headers=headers, rate_limit=rate_limit))
            tasks.append(task)
            offset += batchsize
