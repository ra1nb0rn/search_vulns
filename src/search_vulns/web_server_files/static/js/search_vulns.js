                ...
                if ((productIDs != undefined && productIDs.length != 0) || Object.keys(search_results.vulns).length != 0) {
                    ...
                    if (productIDs.length > 0 && productIDs[0].length > 0) {
                        ...
                        if (productIDs.length > 1) {
                            search_display_html += '<div class="dropdown dropdown-hover dropdown-bottom dropdown-end ml-2"><div class="btn btn-circle btn-outline btn-info btn-xxs"><i class="fa-solid fa-up-right-and-down-left-from-center"></i></div><div class="dropdown-content translate-x-2.5 z-[1] p-3 shadow bg-base-300 rounded-box text-base-content w-56 overflow-wrap sm:text-nowrap sm:w-fit" onclick="document.activeElement.blur();"><h5 class="font-medium text-left text-sm grid grid-cols-[1fr_auto] items-start gap-2"><span>Equivalent product IDs that were included into your search: </span><div class="tooltip tooltip-top text-wrap ml-1" data-tip="Sometimes there are multiple IDs for one product, e.g. because of a rebranding."><i class="fas fa-info-circle text-content text-right"></i></div></h5><ul tabindex="0" class="list-disc pl-6 mt-1 text-left text-sm font-light">';
        ...
        success: function (productIDInfos) {
            if (productIDInfos === null || typeof productIDInfos !== 'object' || Array.isArray(productIDInfos)) {
                ...
                if (allProductIDs.length != 0) {
                    ...
                    for (var i = 0; i < allProductIDs.length; i++) {
                        dropdownContent += `<li tabindex="0" class="my-0 py-0"><a class="break-all" id="product-id-suggestion-${i}" onclick="searchVulnsAction(this)">${htmlEntities(allProductIDs[i])}</a></li>`;
                    ...
                    $('#productIDSuggestions').html(`<div class="p-1">No possible product IDs found</div>`);
                }
            }
            curSelectedProductIDSuggestion = -1
